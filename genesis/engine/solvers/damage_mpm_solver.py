...
class MPMSolver(Solver):
    ...
    def init_particle_fields(self):
        ...
        struct_particle_state = ti.types.struct(
            ...
            # 新增损伤相关字段
            damage=gs.ti_float,
            damage_threshold=gs.ti_float,
            damage_rate=gs.ti_float,
    ...
    def p2g(self, f: ti.i32):
        for i in range(self._n_particles):
            if self.particles_ng[f, i].active:
                # Update 计算应变率（示例：基于速度梯度）
                grad_v = self.particles[f, i].C
                strain_rate = ti.sqrt(0.5 * (grad_v + grad_v.transpose()).norm_sqr())
                # ---- 损伤更新逻辑 ----
                self.particles_info[i].plastic_strain += strain_rate * self.substep_dt
                if self.particles_info[i].plastic_strain > 0.05:  # 塑性应变阈值
                    damage_increment = self.particles_info[i].damage_rate * \
                                 ti.sqrt(strain_rate) * self.substep_dt
                    self.particles_info[i].damage += damage_increment
                # 限制损伤范围 [0,1]
                self.particles_info[i].damage = ti.max(0.0, ti.min(1.0, self.particles_info[i].damage))
                # ---- 失效触发逻辑 ----
                if self.particles_info[i].damage >= self.particles_info[i].damage_threshold:
                    self.particles_ng[f+1, i].active = 0
                    self.particles[f+1, i].vel = ti.Vector.zero(gs.ti_float, 3)
                    self.particles_info[i].damage = 1.0  # 完全失效
                # ---- 损伤相关应力衰减 ----
                damage = self.particles_info[i].damage
                effective_mu = self._mu * (1.0 - damage**2)  # 非线性衰减
                effective_lam = self._lam * (1.0 - damage**1.5)
                # 使用衰减后的模量计算应力（替换原有应力计算）
                stress = effective_mu * (...) + effective_lam * (...)
